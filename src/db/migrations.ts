...
export async function runMigrations() {
  ...
  try {
    ...
    // Add onboarding field to businesses table
    await client.query(`
      DO $$ 
      BEGIN
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                      WHERE table_name='businesses' AND column_name='onboarding_completed_at') THEN
          ALTER TABLE businesses ADD COLUMN onboarding_completed_at TIMESTAMP;
        END IF;
      END $$;
    `);
    ...
    // Create services table (must be before reviews migration that references it)
    await client.query(`
